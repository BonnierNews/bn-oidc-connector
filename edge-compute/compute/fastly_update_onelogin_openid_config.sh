...
OIDC_DISCOVERY_URL="https://bn-login-id-service-lab.bnu.bn.nr/oauth/.well-known/openid-configuration"
JWKS_URL="https://bn-login-id-service-lab.bnu.bn.nr/oauth/jwks"
...
curl -s $OIDC_DISCOVERY_URL | fastly config-store-entry create --store-id=$CONFIG_STORE_ID --key="openid_configuration" --stdin
echo "Fetching JWKS document..."
curl -s $JWKS_URL | fastly config-store-entry create --store-id=$CONFIG_STORE_ID --key="jwks" --stdin
